#!/usr/bin/env python3
"""
Diversify NER training data by limiting name repetition.
Select maximum stories where no name appears in more than N different stories.
Works with NER JSONL format (entities field).
"""
...
def analyze_name_distribution(jsonl_path: str) -> Tuple[Dict[str, Set[int]], int]:
    ...
    with open(jsonl_path, 'r', encoding='utf-8') as f:
        for story_idx, line in enumerate(f):
            ...
            # Extract all unique names from entities
            names_in_story = set()
...
def main():
    parser = argparse.ArgumentParser(
        description="Diversify NER training data by limiting name repetition"
    )
    parser.add_argument(
        '--input',
        required=True,
        help='Input JSONL file (NER format with entities field)'
    )
    parser.add_argument(
        '--output',
        required=True,
    ...
    parser.add_argument(
        ...
        type=int,
        default=15,
        help='Maximum number of stories each name can appear in (default: 15)'
    )

    args = parser.parse_args()

    print("🔍 DIVERSIFYING NER TRAINING DATA BY NAME FREQUENCY")
    print("=" * 70)
    ...
    for threshold in [5, 7, 10, 12, 15, 20, 25]:
